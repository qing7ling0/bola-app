...
export class AllotProvider {
  ...
  sampleAllotApply(doc: any) {
    return this.api.mutationDefault('sampleAllotApply', {doc:doc}, true).then((result)=>{
      if(result.code === 0) {
  ...
  sampleAllotInbound(id: string) {
    return this.api.mutationDefault('sampleAllotUpdate', {_id:id, doc:{status:constants.E_SAMPLE_ALLOT_STATUS.COMPLETED}}, true).then((result)=>{
      if(result.code === 0) {
        return result.data.sampleAllotApply;
      }
      return null;
  ...
  sampleAllotOutbound(id: string, doc:any) {
    ...
    return this.api.mutationDefault('sampleAllotUpdate', {_id:id, doc:doc}, true).then((result)=>{
      if(result.code === 0) {
        return result.data.sampleAllotApply;
      }
      return null;
